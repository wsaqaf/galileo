class ClaimsController < ApplicationController
  ...
  def index
    ...
    if (params[:filter]=="r")
      ...
      qry="NOT EXISTS (SELECT claim_id FROM claim_reviews WHERE claims.id=claim_reviews.claim_id AND ((claim_reviews.review_sharing_mode=1 AND claim_reviews.review_verdict!='') OR claim_reviews.user_id="+current_user.id.to_s+"))"
